...
//#include "lwipopts.h" // TODO: verify the need or not of LWIP_NETIF_TX_SINGLE_PBUF
...
//		}
		...
		if (err < 0) {
			...
			ESP_LOGI(TAG, "Error occurred during sending: errno %d, len: %d", errno, picture->len);
			if(errno == EDQUOT || errno == ENOMEM || errno == EMSGSIZE)
			{
				consecutivesErrors++;
				if(consecutivesErrors < 10)
